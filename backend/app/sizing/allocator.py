...
from app.strategy.params import SizingParams
...
def compute_position_size(
    ...
    # Own capital needed = allocated / leverage
    own_capital_needed = allocated / params.leverage
    own_capital_needed = min(own_capital_needed, current_cash)  # can't exceed cash

    # Gross exposure this trade would add
    qty = max(0, math.floor(own_capital_needed * params.leverage / entry_price))
    
    # ── Check max volume limit ───────────────────────────────────────────
    if avg_volume_20d > 0 and params.max_volume_pct > 0:
        max_shares_by_vol = math.floor(avg_volume_20d * (params.max_volume_pct / 100.0))
        
        # Safety cap: if stock split backwards, volume might be artificially high.
        # Ensure volume limit doesn't allow more than the entire portfolio exposure limit.
        max_portfolio_exposure = current_equity * params.max_exposure_pct / 100.0
        max_theoretical_shares = math.floor(max_portfolio_exposure * params.leverage / entry_price) if entry_price > 0 else 0
        max_shares_by_vol = min(max_shares_by_vol, max_theoretical_shares)
    ...
    if qty == 0:
        return SizingResult(
            ...
            borrowed=0, gross_exposure=0,
            entry_price_with_slip=entry_price,
            skipped=True, skip_reason="Insufficient capital or volume limit restricted to 0 shares"
        )
    ...
    if current_gross_exposure + gross_this_trade > max_allowed_exposure:
        # Try to fit within remaining exposure budget
    ...
    # Final check: enough cash for own capital portion
    if own_capital_used > current_cash + 1.0:  # 1 rupee tolerance
        return SizingResult(
            qty=0, allocated_capital=0, own_capital_used=0,
            borrowed=0, gross_exposure=0,
            entry_price_with_slip=entry_price,
